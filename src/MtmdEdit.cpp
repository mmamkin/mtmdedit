...
AC_IMPLEMENT_EXTENSION_MODULE(MtmdEdit2008DLL)
...
#ifdef NEVER
AFX_EXTENSION_MODULE MtmdEdit2008ExtDLL ={ NULL, NULL } ;
#endif
...
BOOL WINAPI DllMain (HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved) {
	...
	if ( dwReason == DLL_PROCESS_ATTACH ) {
		...
		MtmdEdit2008DLL.AttachInstance (hInstance) ;
		InitAcUiDLL () ;
	} else if ( dwReason == DLL_PROCESS_DETACH ) {
		MtmdEdit2008DLL.DetachInstance () ;
	}
	return (TRUE) ;
}
